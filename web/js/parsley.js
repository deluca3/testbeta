  ...
  ParsleyField.prototype = {
    ...
    */
    , bindValidationEvents: function () {
      ...
      // check if its a input or textarea, bind paste event if so.
      if ( this.$element.is( '[type="text"], textarea' ) ) {
        triggers += new RegExp('paste', 'i').test( triggers ) ? '' : ' paste';
      }
    ...
    */
    , eventValidation: function ( event ) {
        ...
        setTimeout(function() {
          var val = mThis.getVal();

          // do nothing on keypress event if not explicitely passed as data-trigger and if field has not already been validated once
          if ( event.type === 'keyup' && !/keyup/i.test( mThis.options.trigger ) && !mThis.validatedOnce ) {
              return true;
          }

          if ( event.type === 'paste' && !/paste/i.test( mThis.options.trigger ) && !mThis.validatedOnce ) {
              return true;
          }

          // do nothing on change event if not explicitely passed as data-trigger and if field has not already been validated once
          if ( event.type === 'change' && !/change/i.test( mThis.options.trigger ) && !mThis.validatedOnce ) {
              return true;
          }

          // start validation process only if field has enough chars and validation never started
          if ( !mThis.isRadioOrCheckbox && mThis.getLength(val) < mThis.options.validationMinlength && !mThis.validatedOnce ) {
              return true;
          }

          mThis.validate();
        }, (event.type === 'paste' ? 5 : 0));
    }
